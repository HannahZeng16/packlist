...
let S={dep:'2026-11-01', own:{},buy:{},pack:{},done:{},chk:{},star:{},del:{},note:{},open:{},hide:{}};
let V={v:'home'};
let storageOK=true;
...
function load(){
  ...
  for(const k of ['own','buy','pack','done','chk','star','del','note','open','hide'])
...
const marked=i=> i.kind==='buy'?(!!S.pack[i.id]||gotIt(i)) : i.kind==='do'?!!S.done[i.id]
              : i.kind==='avoid'?!!S.chk[i.id] : false;

function vCat(cid){
  const t=TOP[cid]; if(!t) return '<div class="empty">找不到这个分类</div>';
  let h='<div class="wrap">';
  t.sub.forEach(s=>{
    const l=inSub(s.id); if(!l.length) return;
    const hidden=S.hide[s.id];
    if(hidden){ return; }
    const open=S.open[s.id]!==undefined ? S.open[s.id] : !s.fold;
    const cl=l.filter(countable), ok=cl.filter(marked).length;
    h+='<div class="grp" data-open="'+(open?1:0)+'">'
      +'<button class="gh" data-grp="'+s.id+'"><span class="gx">›</span><span class="gn">'+esc(s.n)+'</span>'
      +'<span class="gc">'+ok+'/'+cl.length+'</span></button>';
    if(s.fold && !open) h+='<div class="fold-tip">用不上就折着，点标题展开</div>';
    h+='<div class="body">'+l.map(rowHTML).join('')+'</div></div>';
  });
  const hid=t.sub.filter(s=>S.hide[s.id]);
  if(hid.length) h+='<div class="muted" style="padding:10px 2px">已隐藏 '+hid.length+' 组：'
    +hid.map(s=>'<button data-unhide="'+s.id+'" style="color:var(--accent);padding:2px 4px">'+esc(s.n)+'</button>').join('')+'</div>';
  return h+'</div>';
}
...
function vMode(m){
  ...
  let groups=[];
  if(m==='shop'){
    const pool=live().filter(i=>isGoods(i)&&!gotIt(i));
    const G=[['cn','在国内买'],['sg','到了当地买'],['','还没定在哪买']];
    groups=G.map(([k,n])=>[n, pool.filter(i=>(i.where||'')===k)]);
    h+='<div class="muted" style="margin-bottom:10px">只显示还没买的。买回来一批，在这里一次勾掉。</div>';
  }else if(m==='pack'){
    const pool=live().filter(packable);
    groups=[['随身（漏了当场出事）',pool.filter(i=>i.carry==='cabin')],
            ['托运',pool.filter(i=>i.carry==='checked')]];
    const nb=stats().notReady;
    h+='<div class="muted" style="margin-bottom:10px">还没准备好的会置灰。'
      +(nb?'现在有 <b>'+nb+'</b> 件还没买 / 还没办好。':'要带的都齐了。')+'</div>';
  }else{
    const pool=live().filter(i=>i.kind==='do');
    const t=TOP['todo'];
    groups=t.sub.map(s=>[s.n, pool.filter(i=>i.p===s.id)]);
  }
  let any=false;
  groups.forEach(([n,l])=>{
    if(!l.length) return; any=true;
    const key='m:'+m+':'+n, open=S.open[key]!==false;
    const cl=l.filter(countable), ok=cl.filter(marked).length;
    h+='<div class="grp" data-open="'+(open?1:0)+'">'
      +'<button class="gh" data-grp="'+esc(key)+'"><span class="gx">›</span><span class="gn">'+esc(n)+'</span>'
      +'<span class="gc">'+ok+'/'+cl.length+'</span></button>'
      +'<div class="body">'+l.map(i=>rowHTML(i,m)).join('')+'</div></div>';
  });
  if(!any) h+='<div class="empty">'+(m==='shop'?'要买的都买齐了 🎉':'这里还没有内容')+'</div>';
  return h+'</div>';
}
...
function rowHTML(i,mode){
  ...
  if(mode==='pack'||(!mode&&k==='buy'&&S.pack[i.id])){
    on=S.pack[i.id]?1:0; dis=!ready(i);
    if(dis) cls=' off';
  ...
  if(!mode&&k==='buy'&&S.own[i.id]) chips+='<span class="chip">家里有</span>';
  if(i.where==='sg') chips+='<span class="chip">到当地买</span>';
  ...
  return '<div class="row'+cls+'" data-id="'+i.id+'">'
    +'<button class="ck" data-on="'+on+'" data-ck="'+i.id+'" data-mode="'+(mode||'')+'"'+(dis?' disabled':'')+'>'
    +(on?'✓':'')+'</button>'
    +'<div class="rt" data-open="'+i.id+'"><div class="t">'+esc(i.t)+'</div>'
    +(note?'<div class="n">'+esc(note)+'</div>':'')
    +(dis&&mode==='pack'?'<div class="n" style="color:var(--warn)">'+(i.kind==='buy'?'还没买':'还没办好')+'</div>':'')
    +(chips?'<div class="chips">'+chips+'</div>':'')+'</div>'
    +'<button class="star" data-star="'+i.id+'" data-on="'+(S.star[i.id]?1:0)+'">'+(S.star[i.id]?'★':'☆')+'</button>'
    +'</div>';
}

/* 搜索 */
function searchBox(){
  return '<div class="search"><span style="color:var(--faint)">🔍</span>'
    +'<input id="q" placeholder="搜东西，比如「牙刷」「插头」" value="'+esc(V.q||'')+'"></div>';
}
function vSearch(){
  const q=(V.q||'').trim().toLowerCase();
  let h='<div class="wrap">'+searchBox();
  if(!q) return h+'<div class="empty">输入关键词</div></div>';
  const l=live().filter(i=>(i.t+' '+(i.n||'')+' '+(S.note[i.id]||'')).toLowerCase().includes(q));
  if(!l.length) return h+'<div class="empty">没找到「'+esc(q)+'」<br><br>'
    +'<button class="btn ghost" data-act="addnew" style="width:auto;padding:10px 18px">＋ 自己加一条</button></div></div>';
  h+='<div class="muted" style="margin-bottom:8px">找到 '+l.length+' 条</div>';
...
document.addEventListener('click',e=>{
  ...
  if(el=hit('data-ck')){
    ...
    if(m==='pack'){ S.pack[id]=!S.pack[id]; }
    else if(i.kind==='buy'){ if(S.own[id]) S.own[id]=false; else S.buy[id]=!S.buy[id];
      if(!S.buy[id]&&!S.own[id]) S.pack[id]=false; }
    else if(i.kind==='do') S.done[id]=!S.done[id];
  ...
  if(el=hit('data-star')){ const id=el.dataset.star; S.star[id]=!S.star[id];
    commit(); V.keepScroll=window.scrollY; render(); return; }
  if(el=hit('data-open')){ detail(el.dataset.open); return; }
  if(el=hit('data-grp')){ const k=el.dataset.grp;
    const cur=S.open[k]!==undefined?S.open[k]:!(SUB[k]&&SUB[k].fold);
    S.open[k]=!cur; commit(); V.keepScroll=window.scrollY; render(); return; }
  ...
  if(el=hit('data-mode')){ go({v:'list',m:el.dataset.mode}); return; }
...
document.addEventListener('input',e=>{
  if(e.target.id==='q'){ V.q=e.target.value; V.c=null; V.m='browse';
    const p=window.scrollY; $('#app').innerHTML=vSearch(); window.scrollTo(0,p);
    const q=$('#q'); if(q){ q.focus(); q.setSelectionRange(q.value.length,q.value.length); } }
});

function act(a,el){
  if(a==='back'){ if(V.q!=null){ V.q=null; } else V.c=null; render(); return; }
